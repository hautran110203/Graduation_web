...
interface DemoPresentationViewProps {
  slides: Slide[];
  autoSlide: boolean;
  slideInterval: number;  // (giây)
  bgImage?: string;
  showControls?: boolean;
  isDemo?: boolean;
  /** index điều khiển từ cha (child window) */
  currentIndex?: number;
}
...
const DemoPresentationView: React.FC<DemoPresentationViewProps> = ({
  ...
  const [index, setIndex] = useState<number>(currentIndex ?? 0);

  // Đồng bộ index từ prop (parent control)
  useEffect(() => {
    if (typeof currentIndex === 'number' && slides.length > 0) {
      const clamped = Math.max(0, Math.min(currentIndex, slides.length - 1));
      setIndex(clamped);
    }
  ...
  if (!slides.length) {
    return <div className="text-center text-lg text-gray-500">Không có slide để hiển thị</div>;
  }
  ...
  return (
    <div
      className={`w-screen h-screen flex items-end justify-center bg-black relative overflow-hidden
  ${isDemo ? 'origin-center scale-[0.8]' : ''} transition-transform duration-200`}
