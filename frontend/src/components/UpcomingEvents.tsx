...
const UpcomingEvents: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const res = await axios.get<EventItem[]>('http://localhost:3001/events');

        const now = Date.now();
        const TEN_DAYS_MS = 10 * 24 * 60 * 60 * 1000;

        const filteredSortedTop3 = res.data
          // chỉ lấy sự kiện mà thời điểm bắt đầu còn cách hiện tại >= 10 ngày
          .filter(e => {
            const start = new Date(e.start_time).getTime();
            return start - now >= TEN_DAYS_MS;
          })
          // sắp xếp tăng dần theo thời điểm bắt đầu
          .sort((a, b) =>
            new Date(a.start_time).getTime() - new Date(b.start_time).getTime()
          )
          // lấy 3 sự kiện gần nhất thỏa điều kiện
          .slice(0, 3);

        setEvents(filteredSortedTop3);
      } catch (err) {
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {events.length === 0 && (
          <div className="text-center text-gray-500 mt-8">
            Hiện không có sự kiện nào còn hạn đăng ký từ 10 ngày trở lên.
          </div>
        )}
      </div>
    </section>
