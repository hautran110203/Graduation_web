...
// const EventModal: React.FC<Props> = ({ event, onClose, onSave }) => {
...
//     axios.get('http://localhost:3001/events/locations')
//       .then(res => Array.isArray(res.data) && setLocations(res.data))
//       .catch(err => {
...
//       setForm({
//         title: event.title?.replace(` ${event.unit_code}`, '') || '',
...
//       });
//     } else if (isAdminUnit) {
//       // Đặt sẵn đơn vị cho admin_unit
//       setForm(f => ({ ...f, unit_code: user.unit_code }));
//     }
//   }, [event]);
...
//   const { title, date, start_time, end_time, location, description } = form;
...
//           {/* Chỉ hiển thị dropdown đơn vị nếu là admin */}
//           {isAdmin && (
//             <select
//               name="unit_code"
...
//             value={form.location}
//             onChange={handleChange}
//             placeholder="Địa điểm (chọn hoặc nhập)"
//             className="w-full border px-3 py-2 rounded"
//           />
...
//           <button onClick={handleSubmit} className="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Lưu</button>
//         </div>
//       </div>
//     </div>
//   );
...
const handleSubmit = () => {
  ...
  const payload: any = {
    title: form.title.trim(), // ❗Không gộp đơn vị tại đây
    location_id,
    description,
    start_time: start.toISOString(),
    end_time: end.toISOString(),
  ...
  console.log('[Form Submit] Payload gửi đi (không gộp đơn vị):', payload);
  onSave(payload);
};
